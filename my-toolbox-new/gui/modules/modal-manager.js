...
export class ModalManager {
    ...
    async showParamModal(script) {
        ...
        if (script.parameters && script.parameters.length > 0) {
            ...
            // 记录 showWhen 条件到 formGroup 上，供后续事件处理
            if (param.showWhen) {
                formGroup.dataset.showWhen = JSON.stringify(param.showWhen);
            }
            ...
            // --- showWhen 条件显示逻辑 ---
            // 找出所有有 showWhen 的 formGroup，绑定触发参数的 change 事件
            const updateVisibility = () => {
                modalBody.querySelectorAll('.form-group[data-show-when]').forEach(group => {
                    const conditions = JSON.parse(group.dataset.showWhen);
                    let visible = true;
                    for (const [triggerName, allowedValues] of Object.entries(conditions)) {
                        const triggerInput = document.getElementById(`param-${triggerName}`);
                        if (triggerInput) {
                            visible = allowedValues.includes(triggerInput.value);
                        }
                    }
            ...
            // 给所有被 showWhen 引用的触发参数绑定 change 事件
            const triggerNames = new Set();
            modalBody.querySelectorAll('.form-group[data-show-when]').forEach(group => {
                const conditions = JSON.parse(group.dataset.showWhen);
                Object.keys(conditions).forEach(name => triggerNames.add(name));
            });
            triggerNames.forEach(name => {
                const triggerInput = document.getElementById(`param-${name}`);
                if (triggerInput) {
