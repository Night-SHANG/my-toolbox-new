...
export class IconManager {
    ...
    async loadImageIcon(iconContainer, iconPath, scriptName) {
        // 检查缓存：如果已加载过，直接使用缓存的 base64 数据，不显示沙漏
        if (this.iconsCache.has(iconPath)) {
            const cachedData = this.iconsCache.get(iconPath);
        ...
        try {
            ...
            if (result.success) {
                // 缓存 base64 数据
                this.iconsCache.set(iconPath, result.base64_data);
