...
3.  **启动工具箱**
    安装完成后，关闭上述命令行窗口，然后双击 `start_toolbox.pyw` 文件即可启动程序。(my-toolbox-new工具箱的路径需要纯英文路径)



## 📖 使用指南

- **添加新脚本**: 只需将您的Python脚本（建议每个脚本一个文件夹）放入根目录下的 `scripts` 文件夹内，程序重启后即可自动发现。（需要添加def get_metadata():"""获取脚本元数据"""才能被工具箱识别）
- **管理依赖**: 在UI界面的“虚拟环境管理”部分，您可以创建新的虚拟环境，并为选定的环境安装或卸载依赖。
